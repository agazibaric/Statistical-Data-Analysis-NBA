...
```{r}
playerPlayoffs <- read.csv(file="data/player_playoff.csv", header=TRUE, sep=",")

avgBlocksSeason <- playerSeasons$BLK / playerSeasons$G
avgBlocksPlayoffs <- playerPlayoffs$BLK / playerPlayoffs$G

t.test(avgBlocksSeason, avgBlocksPlayoffs, alt = "two.sided", var.equal = TRUE)

avgFoulsSeason <- playerSeasons$PF / playerSeasons$G
avgFoulsPlayoffs <- playerPlayoffs$PF / playerPlayoffs$G

t.test(avgFoulsSeason, avgFoulsPlayoffs, alt = "less", var.equal = TRUE)

playerSeasonsTmp <- playerSeasons[!is.na(playerSeasons$FG.) & !is.na(playerSeasons$Age) & playerSeasons$Age > 18 & playerSeasons$Age < 41, ]
