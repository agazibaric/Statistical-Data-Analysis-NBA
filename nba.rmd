...
grid()
points(years, correlationsPearson, cex=0.3, col="blue")
...
ft <- rep(0, length(names))

ps <- playerSeasons[!is.na(playerSeasons$X3P) & !is.na(playerSeasons$X3PA) & !is.na(playerSeasons$FTA) & !is.na(playerSeasons$FT) & playerSeasons$X3PA > 50, ]
...
for (i in 1:length(names)) {
  ...
  ft[i] <- sum(ps$FT[ps$Player == names[i]]) / sum(ps$FTA[ps$Player == names[i]])
}
heights1 <- heights[!is.na(trps)]
trps <- trps[!is.na(trps)]
plot(heights1, trps, cex=0.2, xlab="Visina", ylab="3P%")
cor(heights1, trps, method="pearson")

heights2 <- heights[!is.na(ft)]
ft <- ft[!is.na(ft)]
plot(heights2, ft, cex=0.2, xlab="Visina", ylab="FT%")
cor(heights2, ft, method="pearson")

meanHeight <- mean(heights2)
print("Mean height: ")
print(meanHeight)
shortH <- heights2[heights2 < meanHeight]
tallH <- heights2[heights2 > meanHeight]
shortFt <- ft[heights2 < meanHeight]
tallFt <- ft[heights2 > meanHeight]

t.test(shortFt, tallFt, alt = "greater", var.equal = TRUE)
```
